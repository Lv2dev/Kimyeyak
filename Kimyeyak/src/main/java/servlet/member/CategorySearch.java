...
public class CategorySearch extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			// 1. 총 검색결과 갯수를 가져옴
			int resultCount = storeDAO.searchCategoryStoreCount(category, page, pageCount);

			// 2. 현재 페이지에 해당하는 글 목록을 가져옴
			list = storeDAO.searchCategoryStorePageing(category, page, pageCount);
