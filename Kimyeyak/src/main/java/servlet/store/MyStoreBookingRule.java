 ...
 * 가게 예약 규칙을 표시하는 서블릿
 */
@WebServlet("/store/MyStoreBookingRule")
public class MyStoreBookingRule extends HttpServlet {
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			// 예약 규칙 가져오기
			ArrayList<BookingRuleDTO> list = BookingDAO.getInstance().getStoreBookingRuleDTOList(storeDTO.getStoreId());
			
			if(list != null) {
				// request에 저장
				request.setAttribute("bookingRuleList", list);
			}
		...
		//이동
		request.getRequestDispatcher("../store/MyStoreBookingRule.jsp").forward(request, response);
		return;
	}
}
