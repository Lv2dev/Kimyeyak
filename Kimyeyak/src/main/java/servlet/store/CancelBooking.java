 ...
 * 들어온 예약을 캔슬하는 서블릿
 */
@WebServlet("/store/CancelBooking")
public class CancelBooking extends HttpServlet {
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 넘어온 부킹아이디 체크
		if (request.getParameter("bookingId") == null || request.getParameter("bookingId") == "") {
			response.sendRedirect("../store/Main");
			System.out.println("넘어온 부킹아이디 체크1");
			return;
		}
		...
		try {
			//예약취소 수행
			BookingDAO.getInstance().cancelBooking(bookingId);
		} catch (Exception e) {
			e.printStackTrace();
		}
		
		//이동
		response.sendRedirect("../store/MyStoreBooking?storeId=" + storeDTO.getStoreId());
		return;
	}
}
