		...
		<section class="py-5 px-0 mx-0 text-center container">
			<div class="pt-lg-5 pb-lg-3 px-0 mx-0 text-center">
				<div class="col-lg-6 col-md-8 mx-auto my-auto">
					<h1 class="fw-light">내 주변</h1>
					<p class="lead text-muted">반경 ${distance } km 내의 가게를 찾았습니다.</p>
				</div>
			</div>
		</section>
		...
		<div
			...
			<div class="col-12 col-md-7 mx-5 my-5 container row justify-content-center shadow-lg rounded bg-body">
				<c:if test="${ page > pageCount }">
					<div class="col-1"><a href="../member/NearStoreProc?page=${ start - 1 }&distance=${distance}">이전</a></div>
				</c:if>
				...
				<c:if test="${ i != page }">
					<div class="col-1"><a href="../member/NearStoreProc?page=${ i }&distance=${distance}">${ i }</a></div>
				</c:if>
				</c:forEach>
				<c:if test="${ end < pages }">
					<div class="col-1"><a href="../member/NearStoreProc?page=${ end + 1 }&distance=${distance}">다음</a></div>
				</c:if>
			</div>
		</div>
	</main>
